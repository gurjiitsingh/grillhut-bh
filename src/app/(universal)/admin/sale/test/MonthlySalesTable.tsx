...
export default function MonthlySalesTable() {
  ...
  const fetchMonthlySales = async () => {
    try {
      const ref = collection(db, 'monthlyReports');
      ...
      const sales: MonthlySales[] = snapshot.docs.map((doc) => {
        const data = doc.data();
  ...
  const summary = useMemo(() => {
    ...
    const cash = monthlySales.reduce(
      (sum, item) => sum + item.cashCollection,
      0
    );

    const card = monthlySales.reduce(
      (sum, item) => sum + item.cardCollection,
      0
    );

    const upi = monthlySales.reduce(
      (sum, item) => sum + item.upiCollection,
      0
    );

    const wallet = monthlySales.reduce(
      (sum, item) => sum + item.walletCollection,
      0
    );
    ...
    <div className="w-full space-y-6 p-4 md:p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Track monthly revenue and payment collections
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-800">
                {currency(summary.averageMonthlySale)}
              </h2>
            </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm md:p-6">
        ...
        <div className="mb-5">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Collection breakdown across all months
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        ...
        <div className="border-b border-slate-100 px-5 py-5 md:px-6">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Detailed sales information by month
          </p>
        </div>
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <span className="font-semibold text-slate-800">
                      {currency(row.totalSales)}
                    </span>
                  </td>

                  <td className="px-6 py-4 text-right text-slate-600">
                    {currency(row.cashCollection)}
                  </td>

                  <td className="px-6 py-4 text-right text-slate-600">
                    {currency(row.cardCollection)}
                  </td>

                  <td className="px-6 py-4 text-right text-slate-600">
                    {currency(row.upiCollection)}
                  </td>

                  <td className="px-6 py-4 text-right text-slate-600">
                    {currency(row.walletCollection)}
                  </td>
        ...
        {monthlySales.length === 0 && (
          <div className="px-6 py-12 text-center">
            ...
            <p className="mt-1 text-sm text-slate-400">
              Monthly sales data will appear here once available.
            </p>
          </div>
        )}
